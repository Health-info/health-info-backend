...
router.post('/room', async (req, res, next) => {
  try {
    ...
    res.redirect(`/gifchat/room/${newRoom.id}?password=${req.body.password}`);
  } catch (error) {
    console.error(error);
    next(error);
  }
...
router.post('/room/:id/whisperchat', (req, res, next) => {
  try {
    ...
    if(req.app.get('max').get(req.body.whisperto)){ // 방에 들어있음
      ...
      const socketid2 = req.app.get('max').get(req.user.nick).slice(6);
